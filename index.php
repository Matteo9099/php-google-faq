    ...
    <main class="ms-main">
        <div class="container w-75 px-5">
            <?php
                //Titolo e paragrafo
                foreach($faqs as $faq){
                    echo "<h3 class='mb-4'>". $faq['title'] ."</h3>";

                    for($i=0; $i < count($faq['paragraph']); $i++){
                        echo "<p class='pb-2'>".$faq['paragraph'][$i]."</p>";
                    }
                    // subtitolo e subparagrafo
                    // documentazione array_key_exist 'https://www.w3schools.com/php/func_array_key_exists.asp'
                    if(array_key_exists('subtitle', $faq)){
                        echo "<h5 class='mb-4'>". $faq['subtitle'] ."</h5>";

                        for($i=0; $i <count($faq['subparagraph']); $i++){
                            echo "<p>". $faq['subparagraph'][$i] ."</p>";
                        }
                    }
                }
